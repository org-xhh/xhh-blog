...
#### 注意事项
不要用 import/require 导入 public 中的 JS 文件（直接 /js/publicUtils.js 即可）

原因如下：

1.import/require 是模块化导入语法，vite/webpack/ 会把导入的文件当作项目源码模块处理，会去src目录下查找，而不是public；

2.public 中的 JS 是静态全局文件，不是ES模块化文件，没有export导出，无法被模块化导入语法识别。
