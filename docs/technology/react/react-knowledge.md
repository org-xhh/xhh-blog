...
是JS的语法扩展，浏览器本身不能识别，JSX 最终会被编译器（如 Babel）转换为 React.createElement() 调用。
...
// 仅导入类型，编译后移出，避免将类型信息打包进JS
import type { ResType } from './common.ts'
```


## 受控组件
受控组件：表单数据由组件状态（state）驱动
...
```
非受控组件：表单数据由 DOM 节点自身维护，使用 defaultValue 属性，非动态更新
```
...
使函数式组件能够拥有类组件的一些特性，例如状态管理和生命周期方法的使用。
  ...
  hooks 使用规则：
1. 必须使用 useXXX 格式命名
2. 只能在 函数式组件内或其他hook 内调用（也就是顶层调用）
...
```
#### use-context-selector 第三方库 
核心 API：createContext/useContextSelector 可以用来创建 context 和从 context 中选取属性，如果这个属性没有发生变化则不会导致组件发生 re-render。 
```
...
const UserProfile = () => {
  // 只订阅 name 字段，当 age 变化时，此组件不会重新渲染
...
const MyInput = forwardRef<RefProps, ChildProps>((props, ref) => {
