...
## 构建 npm
1.npm i 安装 package.json 中的依赖包

2.微信开发者工具 => 工具 => 构建 npm，自动生成 miniprogram_npm 文件夹
...
管理者后台，左侧菜单管理-版本管理，找到开发版本那一栏，将刚才自己上传的包选为体验版本。
可以有多个开发版本，只能设一个体验版。
...
### 管理-开发管理-接口设置，地理位置 或 其他接口
比如：wx.getLocation 开通后才能在正式版本使用。

注: 审核接口时小程序地址信息相关功能不能报错并且使用顺畅，要能清楚地看到此接口在哪里使用，否则申请描述再详细也会审核不通过。

### 基础功能-订阅消息
![alt text](image-11.png)

小程序前端（用户点击按钮）发起订阅授权 wx.requestSubscribeMessage
