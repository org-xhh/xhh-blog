...
- Cache-Control（优先级高）是 HTTP/1.1 中最重要的缓存控制字段之一，只需要在响应头添加上需要缓存的时间就行了
    ...
    - must-revalidate：一旦缓存过期，必须去服务器验证，不能使用过期的缓存
...
当强缓存失效（比如到 max-age 时间了），浏览器就会携带一些“凭证”去问服务器：“本地这个缓存还能用吗？” ，这个过程就是协商缓存。
...
- 当服务器响应一个资源请求时，可以在响应头中包含 ETag 字段，通常是文件内容的哈希值。
